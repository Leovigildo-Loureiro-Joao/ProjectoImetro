    ...
    private void drawChart() {
        ...
        switch(getMode()) {
            ...
            case SECTOR:
                ...
                chartCtx.translate(-CENTER_X, -CENTER_Y);
                double radius;
                final double sectorStrokeWidth   = clamp(0.5, 2.5, size * 0.004);
                final Color  defaultSectorStroke = getGridColor().deriveColor(0, 1, 1, 0.65);
                // sector mode
                for (int i = 0 ; i < NO_OF_SECTORS ; i++) {
                    ...
                    chartCtx.closePath();
                    if (gradientStops.isEmpty()) {
                        Color fill = data.get(i).getFillColor();
                        if (null != fill) { chartCtx.setFill(fill.deriveColor(0, 1, 1, 0.78)); }
                    }
                    Color stroke = data.get(i).getStrokeColor();
                    if (null == stroke || Double.compare(stroke.getOpacity(), 0.0) == 0) {
                        if (gradientStops.isEmpty()) {
                            Color fill = data.get(i).getFillColor();
                            stroke = null == fill ? defaultSectorStroke : fill.deriveColor(0, 1, 0.7, 0.92);
                        } else {
                            stroke = defaultSectorStroke;
                        }
                    }
                    chartCtx.fill();
                    chartCtx.setStroke(stroke);
                    chartCtx.setLineWidth(sectorStrokeWidth);
                    chartCtx.stroke();

...
public class PerfilController implements Initializable {
    ...
    private void renderMedals() {
        ...
        if (medalhasFlow != null) {
            ...
            for (MedalSupport.MedalViewModel medal : medals) {
                medalhasFlow.getChildren().add(buildMedalCard(medal));
            }
        }
    ...
    private VBox buildMedalCard(MedalSupport.MedalViewModel medal) {
        ...
        VBox card = new VBox(12, artShell, pillRow, titleLabel, descriptionLabel, metaLabel, statusPill, footerLabel);
        card.getStyleClass().addAll("card-blur", "shadow", "medal-card");
        card.getStyleClass().add(medal.unlocked() ? "medal-card-unlocked" : "medal-card-locked");
        card.setPadding(new Insets(16));
        card.setPrefWidth(230);
        card.setMinHeight(288);
